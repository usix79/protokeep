...
type Winner3Way =
    ...
    member x.ItemValues() = [| x.Win1; x.Draw; x.Win2 |]
...
type Handicap =
    ...
    static member MakeKey(value': decimal) = Key.Value(value'.ToString())

    interface IEntity with
        member x.Key = Handicap.MakeKey(x.Value)

    member x.ItemValues() = [| x.Win1; x.Win2 |]

    member x.ItemIndexes() = [| Key.Value "2"; Key.Value "3" |]
...
type Total =
    ...
    member x.ItemValues() = [| x.Over; x.Under |]
...
type Score =
    ...
    static member Default: Lazy<Score> = lazy { S1 = 0; S2 = 0 }
...
type CorrectScore =
    ...
    static member Default: Lazy<CorrectScore> = lazy { Scores = List.empty }
    ...
    member x.WithItems(items: Map<Key, _>) =
        { x with
            Scores =
                x.Scores
                |> List.map (fun v ->
                    items.TryFind v.Score.Key
                    |> Option.map (fun i -> { v with Outcome = i })
