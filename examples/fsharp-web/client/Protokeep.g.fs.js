...
import { ofArray, map as map_1 } from "./fable_modules/fable-library.4.1.4/List.js";
...
import { toList, ofList, FSharpMap__TryFind, empty } from "./fable_modules/fable-library.4.1.4/Map.js";
...
export class FsharpTypes_Key extends Union {
    ...
    toString() {
        const x = this;
        return (x.tag === 1) ? join(",", map_1(toString, x.fields[0])) : ((x.tag === 2) ? (`(${x.fields[0]})`) : x.fields[0]);
    }
}
...
export function FsharpFableHelpers_ifMap(action, _arg) {
    if (_arg.tag === 4) {
        iterate((v) => {
            if (v.tag === 5) {
                ...
                iterate((key) => {
                    iterate((value) => {
                        action([key, value]);
                    }, toArray(FSharpMap__TryFind(v_1, "Value")));
                }, toArray(FSharpMap__TryFind(v_1, "Key")));
            }
        }, _arg.fields[0]);
    }
}

export function FsharpFableHelpers_mapToArray(keyCast, valueCast, map) {
    return new Json(4, [map_1((tupledArg) => (new Json(5, [ofList(ofArray([["Key", keyCast(tupledArg[0])], ["Value", valueCast(tupledArg[1])]]), {
        Compare: comparePrimitives,
    })])), toList(map))]);
}
