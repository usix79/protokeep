...
import { enum_type, record_type, class_type, union_type, list_type, option_type, string_type, int32_type } from "./fable_modules/fable-library.4.1.4/Reflection.js";
import { FsharpTypes_Key } from "./Protokeep.g.fs.js";
...
export class Op extends Union {
    ...
    cases() {
        return ["Unknown", "Val", "Sum", "Mul", "Div", "Ln", "Quantum", "Imagine", "SumAll", "Zero"];
    }
    get Key() {
        const x = this;
        return (x.tag === 1) ? Op_MakeValKey() : ((x.tag === 2) ? Op_MakeSumKey() : ((x.tag === 3) ? Op_MakeMulKey() : ((x.tag === 4) ? Op_MakeDivKey() : ((x.tag === 5) ? Op_MakeLnKey() : ((x.tag === 6) ? Op_MakeQuantumKey() : ((x.tag === 7) ? Op_MakeImagineKey() : ((x.tag === 8) ? Op_MakeSumAllKey() : ((x.tag === 9) ? Op_MakeZeroKey() : Op_MakeUnknownKey()))))))));
    }
}

export function Op_$reflection() {
    return union_type("Domain.Op", [], Op, () => [[], [["p1", int32_type]], [["p1", Op_$reflection()], ["p2", Op_$reflection()]], [["p1", Op_$reflection()], ["p2", Op_$reflection()]], [["p1", Op_$reflection()], ["p2", Op_$reflection()]], [["p1", Op_$reflection()]], [["p1", Op_$reflection()], ["p2", Op_$reflection()], ["p3", string_type]], [["p1", option_type(int32_type)]], [["p1", list_type(Op_$reflection())]], []]);
}
...
export function Op_MakeSumAllKey() {
    return new FsharpTypes_Key(0, ["8"]);
}

export function Op_MakeZeroKey() {
    return new FsharpTypes_Key(0, ["9"]);
}
...
export function OpError_$reflection() {
    return union_type("Domain.OpError", [], OpError, () => [[], [["p1", list_type(string_type)]], [], []]);
}
...
export class Request extends Record {
    constructor(Token, Operation, Tags) {
        super();
        this.Token = Token;
...
export function Request_$reflection() {
    return record_type("Domain.Request", [], Request, () => [["Token", string_type], ["Operation", Op_$reflection()], ["Tags", class_type("Microsoft.FSharp.Collections.FSharpMap`2", [string_type, string_type])]]);
}

export function Request_get_Default() {
    return new Lazy(() => (new Request("", new Op(0, []), empty({
