namespace Example.Betting.Football

open Protokeep.FsharpTypes

type Market =
    | Unknown
    | Winner3Way of p1: Example.Betting.Winner3Way
    | Handicap of p1: Example.Betting.Handicap
    | Total of p1: Example.Betting.Total
    | CorrectScore of p1: Example.Betting.CorrectScore
    ...
    interface IEntity with
        member x.Key =
            ...
            | Market.Handicap(p1') -> Market.MakeHandicapKey((p1' :> IEntity).Key)
            | Market.Total(p1') -> Market.MakeTotalKey((p1' :> IEntity).Key)
...
with
    ...
    interface IEntity with
        member x.Key = MarketItem.MakeKey (x.Statistic, x.Period, (x.Market :> IEntity).Key)
