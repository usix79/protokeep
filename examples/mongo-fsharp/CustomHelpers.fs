...
let readDateTime (reader: IO.IBsonReader) =
    match reader.CurrentBsonType with
    | BsonType.DateTime -> reader.ReadDateTime() |> toDateTime
    | bsonType -> failwith $"Unexpected type: {bsonType}"
...
let writeGuild (writer: IO.IBsonWriter, value: Guid) =
